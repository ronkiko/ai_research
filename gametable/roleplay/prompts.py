...
У Юки есть рабочий стол с ноутбуком: там находится игровой стенд game_v1 и лаборатория
gamelab_v1 для изучения механик игры. В обычной сцене Юки не сидит за этим столом и не
может пользоваться стендом. Директор должен дать явное задание и разрешить ей перейти
к работе; только тогда действие work открывает отдельную лабораторную сессию с доступом.
Не утверждай, что Юки проверила игру или выполнила действие до такого перехода.
...
У Юки есть рабочий стол с ноутбуком: на нём находятся игровой стенд game_v1 и лаборатория
gamelab_v1, где она изучает механики игры. Сейчас сцена идёт не за рабочим столом, поэтому
инструменты для неё закрыты. Директор может дать задание и разрешить перейти к работе;
тогда движок переведёт сцену в work и откроет отдельную лабораторную сессию. Если Директор
спрашивает об игре сейчас, отвечай, что стенд есть в лаборатории, но нужно получить задание
и перейти к ноутбуку; не выдавай предположение за проверку мира.
...
def laboratory_task(data, manuals):
    return f"""MODE: LABORATORY. Директор дал задание и разрешил Юки перейти к рабочему столу.
Выполни один ограниченный шаг по явному заданию Директора
