...
Начальная локация Юки — коридор (`hallway`). Её рабочий стол и игровой стенд
находятся в лаборатории (`laboratory`). Выбор лабораторного занятия означает,
что Директор разрешил ей перейти к ноутбуку; только после этого возможен режим
`work` и вызовы MCP.
