...
LLM. `learning_v1` поддерживает обучение Motor, отдельную one-shot
сертификацию и обучение Spine с проверенным Motor. В GameTable параметры
ограниченного рабочего шага фиксирует сервер, MCP исполняет approval_id.
...
## Series 2: рабочее дерево после аудита 2026-09-27

Добавлен игровой путь отдельных Motor/Spine TRAIN/VERIFY и выбора навыка,
аутентифицированный loopback MCP bridge с immutable approvals, отдельный
workstation interaction, фоновое наблюдение результатов и bounded инициатива
после onboarding. Результаты сохраняются в опыте и передаются голосам. Это
описание реализации, не доказательство живого LLM-поведения или научной сходимости.
Текущие бюджеты практики — 100 эпизодов Motor и 100 Spine; автоматического
PASS сертификации нет. [Протокол передачи и приёмки](docs/reports/series-2-gameplay-acceptance.md)
сохраняет ручные и научные проверки открытыми в Series 2.
