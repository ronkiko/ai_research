...
навигации и обучения через MCP. Этапы 02–03 уже зафиксировали identity/action/world
contracts, map catalog и отдельный `embodied_world_v1` scheduler с multi-zone
physics. Legacy supervisor и текущая VN ещё не переключены на него: публичный
cutover остаётся отдельным этапом. Перенос authority выполняется поэтапно.
