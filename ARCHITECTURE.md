...
навигации и обучения через MCP. Этапы 02–04 уже зафиксировали identity/action/world
contracts, multi-zone physics и канонический learned-body core в `organism/`.
GameLab пока остаётся compatibility facade, legacy supervisor и текущая VN ещё
не переключены на новый стек: публичный cutover остаётся отдельным этапом.
