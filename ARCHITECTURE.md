...
Уточнение цели: [план из 10 коммитов](docs/refactor/embodied-vn/01-concept.md)
объединяет тело Юки и персонажа новеллы, заменяет публичный GameLab интерфейсами
навигации и обучения через MCP. Этот план заменяет направление будущего развития
ниже; описание действующего разделения VN/физического стенда пока остаётся верным.
Перенос authority выполняется по этапам, а не фактом публикации плана.
...
GameLab меняет веса на измеренных последствиях. Во время RUN Brain задаёт
...
| Координата, скорость, усилие, epoch/tick | GameServer Zone | Текст модели не является физическим действием |
| Сессия игрока, последовательность ввода | GameClient Host | Host не определяет успех обучения |
| Веса, сертификаты, эксперименты и их статусы | GameLab | Рассказ об успехе не заменяет VERIFY |
...
`hallway` и `laboratory.workstation` пока являются сценами интерфейса персонажа,
а не геометрией физического стенда. Сидящая Юки за ноутбуком — представление
её работы; движение экспериментального тела доказывается только GameServer.
...
3. Юки не имеет прямого `game_v1_move`: Brain задаёт цель через GameLab.
   Ручные CLI/GUI остаются инструментами оператора; вмешательство в измеряемый
