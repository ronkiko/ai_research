...
LEGACY_MOTOR_ROOT = Path(__file__).resolve().parents[2] / "gamelab" / "motors"
MOTOR_ARCHITECTURE_SCHEMA = 1
MOTOR_INSTANCE_SCHEMA = 1
CURRENT_MOTOR_SCHOOL_VERSION = "velocity_tracking_measured_v8"
...
def motor_root() -> Path:
    configured = os.environ.get("ORGANISM_MOTOR_ROOT") or os.environ.get("GAMELAB_MOTOR_ROOT")
...
def import_legacy_motor_instance(motor_id: str) -> MotorPackage:
    """Copy only a fully compatible certified legacy Motor into organism storage."""
    motor_id = _valid_motor_id(motor_id)
    source = LEGACY_MOTOR_ROOT / "instances" / motor_id
    if not source.is_dir():
        raise MotorPackageError(f"legacy motor {motor_id!r} is not installed")
    source_package = require_trained_motor(_load_instance(source))
    destination = instances_root() / motor_id
    if destination.is_dir():
        return require_trained_motor(_load_instance(destination))
    destination.parent.mkdir(parents=True, exist_ok=True)
    staging_root = Path(
        tempfile.mkdtemp(prefix=f".{motor_id}.import-", dir=destination.parent)
    )
    staging = staging_root / motor_id
    try:
        shutil.copytree(source, staging)
        imported = require_trained_motor(_load_instance(staging))
        if imported.brain_sha256 != source_package.brain_sha256:
            raise MotorPackageError("legacy Motor import changed certified brain bytes")
...
def _direct_motor_package(motor_id: str) -> MotorPackage:
    ...
    if motor_root() == DEFAULT_MOTOR_ROOT:
        return import_legacy_motor_instance(motor_id)
...
def _best_motor_package() -> MotorPackage:
    ...
    if not candidates and motor_root() == DEFAULT_MOTOR_ROOT:
        legacy = LEGACY_MOTOR_ROOT / "instances"
        if legacy.is_dir():
            for path in legacy.iterdir():
                if not path.is_dir() or path.name.startswith("."):
                    continue
                try:
                    package = require_trained_motor(_load_instance(path))
                except MotorPackageError:
                    continue
                candidates.append(package)
    if not candidates:
    ...
    if selected.path.parent == LEGACY_MOTOR_ROOT / "instances":
        return import_legacy_motor_instance(selected.motor_id)
    return selected
