"""MlpModel — нейросеть с одним скрытым слоем через PyTorch.

2 входа → 4 нейрона (ReLU) → 1 выход → sigmoid. Градиенты через autograd,
оптимизатор Adam. Решает XOR (нелинейно разделимую задачу).
"""
...
class MlpModel(Model):
    KEY = "mlp"
    TITLE = "Нейросеть (MLP)"
    SUMMARY = "2 входа → 4 скрытых → 1 выход, PyTorch autograd + Adam"
    N_PARAMS = 0
    LR = 0.05
    ...
    def train(self, obs: Observation, outcome: Outcome) -> None:
        ...
        if self.train_mode == ADAPTIVE:
            ...
            lr = self._lr_base * (_ADAPTIVE_LR_BOOST if self._adaptive_cooloff > 0 else 1.0)
