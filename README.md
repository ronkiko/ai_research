...
sessions, а Will/Ego отдельно оценивает устойчивость намерения и прогнозируемое
поведение под давлением. Периодическая Audience даёт независимую социальную
оценку, а устойчивый Character Core обусловливает все эти модели без таблицы
обязательных реакций. Для значимого личного решения causal runtime заставляет
основной LLM-контекст пройти Heart → Head → Will/Ego и фиксирует результат Will,
не позволяя родительской LLM переписать behavior/agency постфактум. Spine CNN
